...
- `FixtureExecutionMetadata` and `ScenarioAsTest` now carry `agentMode` so OpenAPI before/after fixture executors can quiet fixture HTTP traffic under the same setting (passed through from SpecmaticJUnitSupport).
