...
- Suite-scoped `AgentConsoleLogger` with `ConsoleLogKind.suppressUnderAgent` so HTTP dumps, stub traffic, and fixture detail can be quieted under agent mode while default `consoleLog` call sites keep printing (including scenario summaries via `ContractExecutionListener`).
