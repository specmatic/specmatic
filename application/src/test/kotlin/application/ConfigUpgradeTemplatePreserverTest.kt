...
class ConfigUpgradeTemplatePreserverTest {
    ...
    fun `preserves camel case top level templates in their v3 locations`() {
        ...
        assertThat(upgraded.at("/specmatic/governance/report/outputDirectory").asText()).isEqualTo(reportDirPath)
    }
    ...
    private fun preserveTemplates(originalConfigYaml: String, upgradedConfigYaml: String): JsonNode {
        return TemplatePreservingConfigUpgrade.preserveTemplates(
            rawLegacyConfig = yamlMapper.readTree(originalConfigYaml),
            upgradedConfig = yamlMapper.readTree(upgradedConfigYaml),
        )
    }
