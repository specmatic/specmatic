...
import io.specmatic.core.QueryParameterDeclaration
...
class OpenApiSpecificationParseTest {
    ...
    @Test
    fun `scalar only query parameters with unique wire keys should not create repeated declarations`() {
        val spec = """
            openapi: 3.0.0
            info:
        ...
        assertThat(queryParamPattern.queryParameterDeclarations.groupBy { it.wireKey }.filterValues { it.size > 1 }).isEmpty()
        assertThat(
    ...
    fun `same type collision between standalone query parameter and form exploded object property should be accepted`() {
        ...
        val declarations = queryParamPattern.declarationsFor("type")

        assertThat(declarations.map { it.source.displayName }).containsExactly("info.type", "type")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("type")
        assertThat(declarations.winningDeclaration().source.kind).isEqualTo(QueryParameterSourceKind.ScalarParameter)
    ...
    fun `collision detection should resolve top level scalar query parameter refs before reading names`() {
        ...
        val feature = OpenApiSpecification.fromYAML(spec, "").toFeature()
        val queryParamPattern = feature.scenarios.single().httpRequestPattern.httpQueryParamPattern
        val declarations = queryParamPattern.declarationsFor("age")

        assertThat(declarations.map { it.source.displayName }).containsExactly("info.age", "age")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("age")
    }
    ...
    fun `different type collision between standalone query parameter and form exploded object property should warn in strict parsing`() {
        ...
        assertThat(queryParamPattern.declarationsFor("type").winningDeclaration().source.displayName).isEqualTo("type")
    }

    @Test
    fun `different type collision diagnostic should include every declaration location and winning declaration`() {
        val spec = """
            openapi: 3.0.0
            info:
              title: Colliding Form Exploded Object Query Param
        ...
        assertThat(stdout).doesNotContain("if parsing continues")
        assertThat(queryParamPattern.declarationsFor("type").map { it.source.displayName }).containsExactly("info.type", "filter.type", "type")
    }

    @Test
    fun `different type collision should use standalone scalar when declared last in lenient parsing`() {
        val spec = """
            openapi: 3.0.0
            info:
              title: Colliding Form Exploded Object Query Param
    ...
    @Test
    fun `last declared scalar query parameter should not require object query property`() {
        val spec = """
            openapi: 3.0.0
            info:
    ...
    @Test
    fun `last declared scalar query parameter should not require object query property when matching stub examples`() {
        val spec = """
            openapi: 3.0.0
            info:
              title: Colliding Required Object Property Query Param
    ...
    @Test
    fun `last declared scalar query parameter should not require unwrapped nested object query property`() {
        val spec = nestedStatusCollisionSpec(scalarDeclaredLast = true)

        val feature = OpenApiSpecification.fromYAML(spec, "").toFeature()
        val queryParamPattern = feature.scenarios.single().httpRequestPattern.httpQueryParamPattern
        val declarations = queryParamPattern.declarationsFor("status")
        val result = queryParamPattern.matches(
            HttpRequest(
                "GET",
                "/orders",
        ...
        assertThat(declarations.map { it.source.displayName }).containsExactly("details.status", "status")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("status")
        assertThat(declarations.winningDeclaration().source.kind).isEqualTo(QueryParameterSourceKind.ScalarParameter)
        assertThat(result).isInstanceOf(Result.Failure::class.java)
        assertThat((result as Result.Failure).reportString()).contains("Expected type boolean")
        assertThat(result.reportString()).doesNotContain("mandatory property \"status\"")
    ...
    fun `last declared unwrapped nested object query property should take precedence over scalar query parameter`() {
        ...
        assertThat(declarations.map { it.source.displayName }).containsExactly("status", "details.status")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("details.status")
        assertThat(declarations.winningDeclaration().source.kind).isEqualTo(QueryParameterSourceKind.NestedObjectProperty)
        assertThat(
            queryParamPattern.matches(
                HttpRequest(
                    "GET",
    ...
    @Test
    fun `different type collision should use object property when declared last in lenient parsing`() {
        val spec = """
            openapi: 3.0.0
            info:
              title: Colliding Form Exploded Object Query Param
        ...
        assertThat(declarations.map { it.source.displayName }).containsExactly("type", "info.type")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("info.type")
        assertThat(declarations.winningDeclaration().source.kind).isEqualTo(QueryParameterSourceKind.FormExplodedObjectProperty)
        assertThat(typePattern).isInstanceOf(QueryParameterScalarPattern::class.java)
        assertThat((typePattern as QueryParameterScalarPattern).pattern).isInstanceOf(NumberPattern::class.java)
    ...
    fun `same type collision across multiple form exploded object query params should resolve refs before comparing`() {
        ...
        assertThat(declarations.map { it.source.displayName }).containsExactly("info.age", "filters.age")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("filters.age")
    }
    ...
    @Test
    fun `different type collision across three query parameter declarations should preserve declaration order`() {
        val spec = """
            openapi: 3.0.0
            info:
              title: Colliding Form Exploded Object Query Params
        ...
        assertThat(declarations.map { it.source.displayName }).containsExactly("info.age", "filters.age", "age")
        assertThat(declarations.winningDeclaration().source.displayName).isEqualTo("age")
        assertThat(agePattern).isInstanceOf(QueryParameterScalarPattern::class.java)
        assertThat(resolvedHop((agePattern as QueryParameterScalarPattern).pattern, scenario.resolver)).isInstanceOf(BooleanPattern::class.java)
    }
...
$parameters
    ...
    private fun HttpQueryParamPattern.declarationsFor(wireKey: String): List<QueryParameterDeclaration> {
        return queryParameterDeclarations.filter { it.wireKey == wireKey }
    }

    private fun List<QueryParameterDeclaration>.winningDeclaration(): QueryParameterDeclaration {
        return last()
    }
