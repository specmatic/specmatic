...
internal class HttpPathPatternTest {
    ...
    @Test
    fun `should return failure with appropriate reason when param matches with conflict`() {
        val pattern = HttpPathPattern(listOf(
            URLPathSegmentPattern(StringPattern(), "customerId"),
            URLPathSegmentPattern(ExactValuePattern(StringValue("orders"))),
            URLPathSegmentPattern(StringPattern(), "orderId", conflicts = setOf("all"))
        ), path = "(customerId:string)/orders/(orderId:string)")

        val allResult = pattern.matches("1234/orders/all", Resolver())
        assertThat(allResult).isInstanceOf(Result.Failure::class.java)
        assertThat((allResult as Result.Failure).hasReason(FailureReason.URLPathParamMatchButConflict)).isTrue()
    }

    @Test
    fun `conflict failure reason should take priority over same-structure failure reason`() {
        val pattern = HttpPathPattern(listOf(
            URLPathSegmentPattern(StringPattern(), "customerId"),
            URLPathSegmentPattern(ExactValuePattern(StringValue("orders"))),
            URLPathSegmentPattern(StringPattern(), "orderId", conflicts = setOf("all"))
        ...
        val allResult = pattern.matches("ABC/orders/all", Resolver())
        assertThat(allResult).isInstanceOf(Result.Failure::class.java)
        assertThat((allResult as Result.Failure).hasReason(FailureReason.URLPathParamMatchButConflict)).isTrue()
    }
