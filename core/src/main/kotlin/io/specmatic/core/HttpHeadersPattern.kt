...
data class HttpHeadersPattern(
    ...
    fun matchContentType(parameters: Pair<Map<String, String>, Resolver>):  MatchingResult<Pair<Map<String, String>, Resolver>> {
        ...
        val contentTypeFromRequest = headers.entries.find { it.key.lowercase() == "content-type" }?.value
    ...
    private fun contentTypeSpecificity(contentType: String): Int {
        val wildcardSpecificity = runCatching { ContentType.parse(contentType) }.getOrNull().let { parsedContentType ->
            when {
                ...
                parsedContentType?.contentSubtype == "*" -> -1
                else -> 0
            }
        }

        val parametersSpecificity = runCatching {
            ContentType.parse(contentType).parameters.size
        }.getOrElse {
    ...
    private fun contentTypeMatchingSource(contentTypeHeaderValueFromRequest: String?, resolver: Resolver): ContentTypeMatchingSource? {
        ...
        return when {
            ...
            contentType != null && contentTypeHeaderValueFromRequest != null -> ContentTypeMatchingSource.MediaType(contentType)
            else -> null
        }
    }
