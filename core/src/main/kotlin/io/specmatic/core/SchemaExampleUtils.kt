...
fun resolveExample(example: String?, pattern: Pattern, resolver: Resolver): Value? {
    ...
    if (exampleMatchResult.isSuccess()) return value
...
fun resolveExample(example: String?, pattern: List<Pattern>, resolver: Resolver): Value? {
    ...
    return matchResults.firstOrNull { it.first.isSuccess() }?.second ?: throw ContractException(
...
fun theDefaultExampleForThisKeyIsNotOmit(valuePattern: Pattern): Boolean {
    ...
    if (patternToValidate !is HasDefaultExample) return true
    val example = patternToValidate.example
    if (example is String) return example !in OMIT
    return true
}
