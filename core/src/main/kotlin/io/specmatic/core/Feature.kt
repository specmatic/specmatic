...
data class Feature(
    ...
    private fun getBadRequestsOrDefault(
        ...
        val badRequestResponses = scenariosToLookInto.filter {
            it.httpRequestPattern.httpPathPattern!!.originalPath() == scenario.httpRequestPattern.httpPathPattern!!.originalPath()
    ...
    private fun convergeRequestPayload(baseScenario: Scenario, newScenario: Scenario): Scenario {
        ...
        return if (baseScenario.httpRequestPattern.formFieldsPattern.size == 1) {
            if (newScenario.httpRequestPattern.formFieldsPattern.size != 1)
                throw ContractException("${baseScenario.httpRequestPattern.method} ${baseScenario.httpRequestPattern.httpPathPattern?.originalPath()} exists with different form fields")
            ...
            if (isObjectType(resolvedBasePattern) && !isObjectType(resolvedNewPattern))
                throw ContractException("${baseScenario.httpRequestPattern.method} ${baseScenario.httpRequestPattern.httpPathPattern?.originalPath()} exists with multiple payload types")
            ...
            val converged: Pattern = when {
                resolvedBasePattern.pattern is String && builtInPatterns.contains(resolvedBasePattern.pattern) -> {
                    if (resolvedBasePattern.pattern != resolvedNewPattern.pattern)
                        throw ContractException("Cannot converge ${baseScenario.httpRequestPattern.method} ${baseScenario.httpRequestPattern.httpPathPattern?.originalPath()} because there are multiple types of request payloads")
                ...
                baseRawPattern is DeferredPattern -> {
                    if (baseRawPattern.pattern == newRawPattern.pattern && isObjectType(resolvedBasePattern))
                        ...
                        throw ContractException("Cannot converge different types ${baseRawPattern.pattern} and ${newRawPattern.pattern} found in ${baseScenario.httpRequestPattern.method} ${baseScenario.httpRequestPattern.httpPathPattern?.originalPath()}")
                }

                else ->
                    TODO("Converging of type ${resolvedBasePattern.pattern} and ${resolvedNewPattern.pattern} in ${baseScenario.httpRequestPattern.method} ${baseScenario.httpRequestPattern.httpPathPattern?.originalPath()}")
            }
    ...
    fun toOpenApi(): OpenAPI {
        ...
        val urlPrefixMap = toOpenAPIURLPrefixMap(scenarios.mapNotNull {
            it.httpRequestPattern.httpPathPattern?.originalPath()
        }.map {
            OpenApiPath.from(it).normalize().toPath()
        })

        val payloadAdjustedScenarios: List<Scenario> = scenarios.map { rawScenario ->
            val prefix = urlPrefixMap.getValue(OpenApiPath.from(rawScenario.httpRequestPattern.httpPathPattern?.originalPath()!!).normalize().toPath())
        ...
        val paths: List<Pair<String, PathItem>> = rawCombinedScenarios.fold(emptyList()) { acc, scenario ->
            ...
            val pathParameters = scenario.httpRequestPattern.httpPathPattern?.pathParameters().orEmpty()
    ...
    private fun toPathPatternWithParameters(httpPathPattern: HttpPathPattern): HttpPathPattern {
        if (httpPathPattern.getPathSegmentPatterns().any { it.pattern !is ExactValuePattern }) return httpPathPattern
        return OpenApiPath.from(httpPathPattern.originalPath()).normalize().toHttpPathPattern()
    }
